...
### Προστέθηκε
- **Κεντρικός Κόμβος Ελέγχου Επιφάνειας Εργασίας** (`src/utils/tray_icon.py`): Το συνοδευτικό pystray επεκτάθηκε σε επιφάνεια ελέγχου επτά στοιχείων -- Άνοιγμα 3D Visualizer, Άνοιγμα Φακέλου Αναφορών, Άνοιγμα Καταγραφής Συστήματος, Άνοιγμα Τεκμηρίωσης API (Swagger), Ενεργοποίηση Άμεσου Κύκλου Αναζήτησης, Εμφάνιση / Απόκρυψη Παραθύρου Κονσόλας και Τερματισμός Δαίμονα. Ο τίτλος εργαλείου μετονομάστηκε σε `"TALOS v5.10.13 | Research Intelligence Mesh"`.
- **Αυτοθεραπευόμενη Αυτόματη Εκκίνηση** (`src/utils/tray_icon.py`): Η νέα `_is_api_alive(port=8001)` ελέγχει το `http://127.0.0.1:8001/api/v1/health` με χρονικό όριο 0.6s; η νέα `_ensure_api_server()` εντοπίζει δυναμικά τη ρίζα του έργου και εκκινεί το `uvicorn src.api.main_api:app --host 127.0.0.1 --port 8001` σε κρυφή διεργασία παρασκηνίου (`subprocess.CREATE_NO_WINDOW` στα Windows), ελέγχοντας μέχρι να αποκριθεί (έως 3s). Οι ενέργειες Visualizer, Swagger και άμεσης αναζήτησης αυτοθεραπεύουν το backend πριν το άνοιγμα.
- **Γέφυρα Επιφάνειας Εργασίας Εγγενούς Λειτουργικού** (`src/utils/tray_icon.py`): Διασταυρωμένα ανοίγματα διαδρομών (`os.startfile` στα Windows, `open` στο macOS, `xdg-open` στο Linux) για τον φάκελο αναφορών (`data/reports`) και την καταγραφή συστήματος (`data/logs/talos_system.log`).

### Άλλαξε
- **Ενιαίο Σημείο Αλήθειας Εμμονής Βάσης Δεδομένων (BREAKING)** (`src/core/database_manager.py`): Ο `DatabaseManager.__init__` πλέον προεπιλέγει το `db_path=None` σε `get_active_profile_db_path()` (τη βάση ενεργού προφίλ στο `_profiles/<active>/talos_research.db`). Αφαιρέθηκε η προηγούμενη προτεραιότητα `data/talos_research.db` και η κληροδοτημένη λογική `_resolve_profile_db`. Μετεγκατάσταση: κάθε στοιχείο που βασιζόταν στη βάση `data/` πρέπει πλέον να παρέχει ρητό `db_path`, διαφορετικά τα δεδομένα καταγράφονται στη βάση ενεργού προφίλ.
- **Αμφίδρομη 4-Καταστάσεων Παραβολική Τηλεμετρία & Μηχανή Συνθετικού Αναγνωριστικού** (`src/api/main_api.py` + `src/integration/visualizer_bridge.py`): Τεκμηριώθηκε η γέφυρα δεσμών 4 καταστάσεων (query_out / data_in / evaluation / error) που αντιστοιχίζει την τηλεμετρία σε υγεία ανά πηγή και παραβολικές δέσμες λέιζερ, καθώς και η συνθετική επικάλυψη τελευταίας αξιολόγησης (`_live_eval_seq` / `_live_eval_state`) που παράγει αυστηρά αύξοντα αναγνωριστικά.
- **Ξεδίπλωμα Ένθετου Τίτλου XML/JSON OpenAIRE** (`src/ingestion/openaire_source.py`): Η `_first()` πλέον ξεδιπλώνει ένθετους περιτυλιχτές λεξικών `$` / `#text` / `value` ώστε τίτλοι όπως `[{"$": "Some Title"}]` να αναλύονται ως απλή συμβολοσειρά.
- **Συμμόρφωση Πρωτοκόλλου Μηδενικών Emojis** (`src/utils/db_stats.py`): Αντικαταστάθηκαν τα emoji στα μετρικά κονσόλας με απλούς δείκτες αγκυλών (`[PAPERS]`, `[ELITE]`, `[AVG]`, `[OK]`, `[WARN]`, `[EMBED]`) ώστε η αναφορά στατιστικών να εκτελείται σε ελληνικές κονσόλες Windows (cp1253) χωρίς UnicodeEncodeError.
- **Επαγγελματική Ανανέωση Διαμόρφωσης Περιβάλλοντος** (`example.env` + `.env`): Ανακατασκευή και των δύο αρχείων σε έξι ενότητες με σχόλια -- (1) Πίνακας Εκτέλεσης & Στρατηγική Δικτύου, (2) Επίπεδα Τοπικών Μοντέλων AI, (3) Καθολικό Νέφος (9 πάροχοι), (4) 16 Ακαδημαϊκά API Κατάποσης, (5) Ενσωματώσεις Οικοσυστήματος, (6) Ειδοποιήσεις Συστήματος. Τα υπάρχοντα μυστικά του `.env` διατηρήθηκαν αυτούσια.
- **Συγχρονισμός συμβολοσειρών έκδοσης σε 6 αρχεία κώδικα** (`config/settings.py`, `src/api/main_api.py`, `talos.py`, `run_talos.bat`, `run_talos.sh`, `tests/test_multi_tier.py`) καθώς και `docker-compose.yml` (`talos:5.10.13`) και `CITATION.cff` σε v5.10.13.
- **Επαναπροσδιορισμός οδικού χάρτη**: DSPy PRISMA Pipeline στην v5.10.14; CORTEX & n8n Gateway στην v5.10.15.
...
- `python -m pytest tests/test_multi_tier.py -k test_talos_version` πέρασε με ισχυρισμό v5.10.13.
- `python src/utils/db_stats.py` επιβεβαίωσε τους αριθμούς εργασιών στη βάση ενεργού προφίλ.
...
### Προστέθηκε
- **Ενσωματωμένη βιβλιοθήκη Three.js** (`static/js/three.min.js`): Πλήρης ελαχιστοποιημένη δέσμη Three.js r128 (UMD, άδεια MIT) που φιλοξενείται τοπικά και εξυπηρετείται από `/static/js/three.min.js` μέσω νέας ταυτοδύναμης προσάρτησης `app.mount("/static", StaticFiles(directory="static"), name="static")` στο `src/api/main_api.py`. Μηδενικές κλήσεις CDN, πλήρης συμμόρφωση air-gapped.
- **Τρισδιάστατος Οπτικοποιητής Three.js** (`templates/live_foraging_visualizer.html`): Ανακατασκευάστηκε από το πειραματικό raw WebGL 1.0 (v5.10.10) σε ισχυρή αρχιτεκτονική Three.js. Χαρακτηριστικά:
  - Φόντο Deep Slate/Navy (`#0f1117`) με Ακαδημαϊκή Λειτουργία Εκτύπωσης (`renderer.setClearColor(0xffffff)`) για στιγμιότυπα δημοσίευσης.
  - Χρυσός (`#f39c12`) πυρήνας `THREE.IcosahedronGeometry` με συνεχή περιστροφή.
  - 16 κυανοί (`#00ced1`) δορυφορικοί κόμβοι με σφαιρική κατανομή Fibonacci (ακτίνα 4.5).
  - Φωτοστέφανα υγείας (`THREE.Sprite`) με απαλή ακτινική υφή: Πράσινο (`#10b981`, Υγιές), Κεχριμπάρι (`#f59e0b`, Ψύξη), Κόκκινο (`#ef4444`, Σφάλμα/403), Κυανό (`#06b6d4`, Ετοιμότητα).
  - Γραμμές σύνδεσης πυρήνα-κόμβων και αχνό πλέγμα αστερισμού γειτόνων.
  - Δέσμη λέιζερ ενέργειας με εξασθένιση 1.2 δευτερολέπτων.
  - Χειριστήρια κάμερας, εναλλαγή THEME, διπλή κατάσταση (SSE / Αναπαραγωγή Συνεδρίου), κάρτα HUD υαλομορφισμού.
- **Ανθεκτική γέφυρα ζωντανής δειγματοληψίας**: Δειγματοληψία 1.5s στο `GET /api/v1/visualizer/demo-data` παράλληλα με το SSE.

### Άλλαξε
- **Συγχρονισμός συμβολοσειρών έκδοσης σε 6 αρχεία κώδικα** σε v5.10.11.
- **Συγχρονισμός τεκμηρίωσης σε 15 κανονικά αρχεία** σε v5.10.11 (2026-08-24).
- **Επαναπροσδιορισμός οδικού χάρτη**: DSPy PRISMA στην v5.10.12; CORTEX & n8n Gateway στην v5.10.13.
- **Ο οπτικοποιητής WebGL 1.0 (v5.10.10) καταγράφεται ως αντικατασταθείς** από την παραγωγική υλοποίηση Three.js.

### Επαλήθευση
- `python -m compileall src config tests talos.py` πέρασε με μηδέν σφάλματα.
- `python -m pytest tests/test_system_integrity.py -q` πέρασε.
- `python -m pytest tests/test_multi_tier.py -k test_talos_version` πέρασε με ισχυρισμό v5.10.11.

## [v5.10.10] - 2026-08-24 -- Τρισδιάστατος Ολογραφικός Αστερισμός Γνώσης & Ζωντανός Οπτικοποιητής Πολλαπλών Σωληνώσεων

### Προστέθηκε
- **Τρισδιάστατος Ολογραφικός Αστερισμός Γνώσης** (`templates/live_foraging_visualizer.html`): Αυτόνομο, μονο-αρχειακό, απομονωμένο διαδραστικό τρισδιάστατο οπτικοποιητή WebGL 1.0 με μηδενικές εξωτερικές εξαρτήσεις δικτύου. Χαρακτηριστικά: Φόντο Deep Slate/Navy ( `#0f1117` ) με πεδίο σωματιδίων περιβάλλοντος χώρου (800+ σωματίδια), δακτύλιοι τροχιακών πλεγμάτων IEEE blue ( `#006699` ) σε 3 επίπεδα κλίσης, περιστρεφόμενος χρυσός ( `#f39c12` ) εικοσάεδρος ως κεντρικός πυρήνας TALOS, 16 κυανοί ( `#00ced1` ) δορυφορικοί κόμβοι-πηγές τοποθετημένοι σε τροχιακούς δακτυλίους, κινούμενες δέσμες ενέργειας που ταξιδεύουν κατά μήκος τρισδιάστατων διανυσμάτων από τον πυρήνα προς τον κόμβο-πηγή-στόχο, κεχριμπαρένια/κόκκινα παλλόμενα κλουβιά κλειδώματος σε σχήμα οκτάεδρου, επικάλυψη HUD υαλομορφισμού με κάρτα αξιολόγησης σε πραγματικό χρόνο, διαδραστικά χειριστήρια κάμερας, λειτουργία διπλής κατάστασης (Ζωντανή Ροή SSE έναντι Αναπαραγωγής Συνεδρίου Χωρίς Σύνδεση), χειριστήρια αναπαραγωγής και φίλτρο βαθμολογίας.
- **Τερματικά Σημεία FastAPI Οπτικοποιητή** (`src/api/main_api.py`): `GET /api/v1/visualizer/live`, `GET /api/v1/visualizer/stream` (SSE), `GET /api/v1/visualizer/demo-data`. Βοηθός `broadcast_visualizer_event()` στη μνήμη με χρήση `queue.Queue`.
- **Σύνδεση Γεγονότων Πολλαπλών Σωληνώσεων**: Τα `daily_search.py`, `historic_search.py` εκπέμπουν γεγονότα `paper_evaluated`. Το `talos.py` `_menu_architecture_graphs()` αποκτά επιλογή "3. 3D Knowledge Constellation Visualizer (Browser)".

### Άλλαξε
- **Αριθμός τερματικών σημείων API** αυξήθηκε από 19 σε 22.
- **Συγχρονισμός συμβολοσειρών έκδοσης σε 6 αρχεία κώδικα και 15 αρχεία τεκμηρίωσης** σε v5.10.10.
- **Ακαδημαϊκή επισημοποίηση της σουίτας δοκιμών**: Το `tests/test_smoke.py` μετονομάστηκε σε `tests/test_system_integrity.py`, και ορίστηκε επίσημα ως η «TALOS Αυτοματοποιημένη Σουίτα Επαλήθευσης Ακεραιότητας Συστήματος» (συμμόρφωση ISO/IEC 25010) για δημοσίευση IEEE και αυστηρότητα παρουσίασης HOU ICBE.

### Επαλήθευση
- `python -m compileall src config tests talos.py` πέρασε με μηδέν σφάλματα.
- `python -m pytest tests/test_system_integrity.py -q` πέρασε.
- `python -m pytest tests/test_multi_tier.py -k test_talos_version` πέρασε με ισχυρισμό v5.10.10.
## [v5.10.9] - 2026-08-23 -- Ολοκληρωμένος Έλεγχος Λειτουργιών TUI & Αποκατάσταση Διαχείρισης Προφίλ

### Προστέθηκε
- **Ιεραρχική Αναδιάρθρωση TUI** (`talos.py`): Μενού 16 επιλογών προοδευτικής αποκάλυψης σε έξι οπτικές ομάδες, αντικαθιστώντας το παλαιό επίπεδο μενού. Κάθε ομάδα ανοίγει ένα ειδικό υπομενού με πλοήγηση Επιστροφής στο Κύριο Μενού και ασφάλεια Ctrl+C μέσω `safe_select()`/`safe_pause()`.
  - **Διαμόρφωση Πυρήνα & Προφίλ** (Επιλογές 1-3): Διαχείριση Μοντέλων AI, Διαχείριση Προφίλ (Εναλλαγή/Δημιουργία/Προβολή), Ερευνητική Εστίαση & Μετάφραση Ερωτημάτων (PYTHIA/Pivot).
  - **Σωληνώσεις Αναζήτησης & Κατάποσης** (Επιλογή 4): Ημερήσια Αναζήτηση (16 APIs), Ιστορική Αναζήτηση Αρχείου, Εξορύκτης Γκρίζας Βιβλιογραφίας, Μαζική Λήψη PDF.
  - **Προηγμένη Ανάλυση & Ευρήματα** (Επιλογές 5-7): Βιβλιογραφική & Επιστημομετρική Ανάλυση (Αναλυτής Αναφορών, Γεννήτρια Μονοπατιών Γνώσης, Σύστημα Συστάσεων, Προφίλ Συγγραφέων, Αναλυτής Τάσεων, Διαδραστικός Πίνακας, Εκπαίδευση DRL), Γραφήματα Αρχιτεκτονικής Κώδικα (Graphify AST + Παλαιό D3.js), Οπτικοποιήσεις Δεδομένων (μέσω OPTICA).
  - **Ενισχυτική Μάθηση & Δαίμονες** (Επιλογές 8-12): Αυτόνομος Ερευνητικός Δαίμονας, Ζωντανός Πράκτορας DRL, Αυτόνομος Κόκκινος Ελεγκτής, Βελτιστοποίηση Σμήνους GWO (Ρυθμιστής Υπερπαραμέτρων, Διαμορφωτής Ανταμοιβών LLM, 3D Ζωντανός Πίνακας), Διαμόρφωση Δαίμονα & Αυτόματης Εκκίνησης ΛΣ.
  - **Σύστημα, Βάση Δεδομένων & Διαγνωστικά** (Επιλογές 13-15): Συντήρηση Βάσης (Εμπλουτισμός Μεταδεδομένων, Επανυπολογισμός Βαθμολογιών, Επαναξιολόγηση Βάσης, Στατιστικά Υγείας ΒΔ), Αναφορές Βάσης (Τυπική/Ακαδημαϊκή 600 DPI), Διαγνωστικά Συστήματος (Κατάσταση DRL, Γεννήτρια Τεκμηρίωσης, Έλεγχος Υγείας API).
- **Αποκατάσταση Διαχείρισης Προφίλ ως κόμβος πρώτης κατηγορίας TUI** (`talos.py` `_manage_profiles()`): Εναλλαγή ενεργού προφίλ, δημιουργία νέου προφίλ με ρύθμιση PYTHIA, προβολή πληροφοριών προφίλ, αποθήκευση τρέχουσας κατάστασης.
- **Εννέα ειδικές συναρτήσεις χειρισμού υπομενού** στο `talos.py`: `_manage_profiles`, `_menu_research_focus`, `_menu_search_ingestion`, `_menu_literature_analysis`, `_menu_architecture_graphs`, `_menu_gwo_suite`, `_menu_database_maintenance`, `_menu_baseline_reports`, `_menu_system_diagnostics`.

### Άλλαξε
- **Εκκαθάριση emoji από τον Διαχειριστή Προφίλ** (`src/core/profile_manager.py`): Όλοι οι χαρακτήρες emoji αντικαταστάθηκαν με δείκτες απλού κειμένου (`[SAVED]`, `[LOADED]`, `[NEW]`, `[ACTIVE]`, `[Saving]`, `[Loading]`). Συμμόρφωση με το Σύνταγμα I (Πρωτόκολλο Μηδενικών Emoji).
- Το **ενοποιημένο θέμα TUI** διατηρείται σε όλους τους νέους χειριστές.
- Η **διοχέτευση DSPy PRISMA** μεταφέρθηκε στην v5.10.11. Η CORTEX & n8n Gateway στην v5.10.12.
- **Συγχρονισμός συμβολοσειρών έκδοσης** σε 6 αρχεία κώδικα και 15 κανονικά αρχεία τεκμηρίωσης σε v5.10.9.
