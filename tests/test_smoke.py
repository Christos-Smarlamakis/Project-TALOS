...
Now lives in tests/ -- chdir's to project root before running.
"""
import sys
import os
import py_compile
...
import json
# -- config.json is at project root (one level up from tests/) --
...
# -----------------------------------------------------------------------------
# 5. QUESTIONARY PATCHING (Interactive Prompt Sandbox)
# -----------------------------------------------------------------------------
print("\n" + "=" * 60)
print("  5. QUESTIONARY PATCHING")
print("=" * 60)

def test_questionary_patch():
    import importlib
    import questionary
    # Simulate what the CLI runner does when patching interactive prompts
...
check("questionary.text patch works", test_questionary_patch)
...
# -----------------------------------------------------------------------------
# 8. STREAMLIT IMPORT (legacy dependency check)
# -----------------------------------------------------------------------------
print("\n" + "=" * 60)
print("  8. STREAMLIT IMPORT (legacy)")
print("=" * 60)

try:
    import streamlit as st
    check("streamlit imported OK", lambda: st.__version__)
    check("st.set_page_config works from import", lambda: None)
except ImportError:
    skip("streamlit import", "streamlit not installed (deprecated)")
...
print(f"  [INFO]  {total} total checks")
print(f"  [INFO]  {len(PY_FILES)} .py files syntax-checked")
print(f"  [INFO]  DB: {stats['total_papers']} papers, avg score {stats['avg_score']:.1f}")
print("=" * 60)

if FAIL == 0:
    print("\nALL CHECKS PASSED -- Project is healthy!")
else:
    print(f"\n{FAIL} checks FAILED -- see details above.")
