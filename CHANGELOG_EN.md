...
## [v5.8.0] - 2026-07-31 -- Multi-Tier LLM Routing, Cross-Platform POSIX Launcher, Isolated Interim UI, Pytest Coverage

### Added -- Multi-Tier LLM Routing Architecture
- `config/settings.py` (NEW): Canonical configuration hub. Defines `FAST_EDGE_MODEL` ("fermionresearch/Neutrino-8B"), `FAST_EDGE_BASE_URL` ("http://127.0.0.1:11435/v1"), `HEAVY_REASONING_MODEL` ("qwen2.5:14b"), `OLLAMA_BASE_URL` ("http://127.0.0.1:11434"), `TALOS_VERSION` ("5.8.0"), `TALOS_API_PORT` (8001), `SYNAPSE_BUS_URL`.
...
### Changed
- `.clinerules`: Version v5.8.0 -> v5.8.0. Footer updated.
- `docs/TIMELINE_EN.md` + `docs/TIMELINE_GR.md`: Phase 3 added with v5.8.0 milestones.
- `docs/PROJECT_MAP_EN.md`: Version bumped to v5.8.0.
- `run_talos.bat`: Version bumped to v5.8.0.
...
- `.clinerules` COMPLETELY REWRITTEN from v5.6.0 12-file rule to v5.8.0 8-Point Constitution v2.0.
...
- `docs/TIMELINE_EN.md` (NEW): Authoritative historical record of all TALOS development milestones in English.
  ...
  - Phase 2 (v5.8.0): 3 milestones marked Complete, 1 unchecked (Python file docstring refactoring).
...
- `src/api/main_api.py`: Version string 5.6.0 -> 5.8.0 across app, docstring, and startup log.
...
- `src/api/main_api.py`: Module docstring fully rewritten to v5.8.0 standard with Dependencies section.
...
- `README.md`: v5.6.0 -> v5.8.0. Tagline updated with SYNAPSE mention. Citation versions bumped.
- `ROADMAP.md`: v5.6.0 -> v5.8.0. New Section 9 for v5.7.x. Summary table updated.
- `docs/PROJECT_MAP.md` footer: v5.6.0 -> v5.8.0, 63 -> 67 files covered.
- `docs/PROJECT_MAP_EN.md` footer: v5.6.0 -> v5.8.0, 63 -> 67 files covered.
...
- **Why `run_talos.bat` instead of updating `start_talos.bat`?** The legacy script is a 7-option Streamlit/CLI launcher tied to legacy architecture. The new script is purpose-built for the v5.8.0 headless API era with a clean 3-option interface.
