# -*- coding: utf-8 -*-
"""
Module: app.py (Streamlit Web GUI v5.4.1 — src/ Migrated)
...
Description:
    ...
    v5.4.1: All imports and script paths migrated to src/ package layout.
...
def render_sidebar():
    with st.sidebar:
        ...
        <p style="color:var(--muted);font-size:.7rem;margin:.2rem 0 0">{t('sidebar_title')} v5.4.1</p>
...
def main():
    ...
    if not st.session_state.advanced_mode:
        ...
        <p style="color:var(--muted);font-size:.85rem">Research Intelligence Platform v5.4.1</p>
        </div>""", unsafe_allow_html=True)
    ...
    TALOS v5.4.1 · © 2026 Christos Smarlamakis · {datetime.now().strftime('%Y-%m-%d %H:%M')}
    </div>""", unsafe_allow_html=True)
