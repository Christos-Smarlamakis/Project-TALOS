...
based on a user's learning goal.It retrieves relevant academic papers from a database, structures them into foundational, 
thematic, and state-of-the-art categories, synthesizes a narrative report using AI, and saves the report in a markdown format.
"""
import json
import os
import sys
...
class KnowledgePathGenerator:
    ...
    It retrieves relevant academic papers, structures them into categories, synthesizes a narrative report, and saves the report.
    Attributes:
        config (dict): Configuration settings for the generator.
        db_manager (DatabaseManager): Instance of the DatabaseManager for database interactions.
        ai_manager (AIManager): Instance of the AIManager for AI-related tasks.
        reports_dir (str): Directory path where generated reports will be saved.
    Methods:
        _get_user_goal(): Prompts the user for their learning goal.
        _find_relevant_papers(goal_text, top_k): Retrieves relevant papers from the database based on the user's goal.
        _extract_keywords_for_filename(goal_text): Extracts keywords from the user's goal for use in the report filename.
        _get_top_keywords_for_cluster(vectorizer, kmeans_model, cluster_id, top_n): Identifies the top keywords for a specific cluster of papers.
        _structure_knowledge(df, num_clusters, min_score): Structures the retrieved papers into foundational, thematic, and state-of-the-art categories.
        _synthesize_narrative(structured_knowledge, user_goal): Synthesizes a narrative report based on the structured knowledge and user's goal.
        _save_report(topic_keywords, narrative_report): Saves the synthesized narrative report to a markdown file.
    """

    def __init__(self, config: dict):
        """
        Initializes the KnowledgePathGenerator with the provided configuration.
        Args:
            config (dict): Configuration settings for the generator.
        """
        self.config = config
    ...
    def _get_user_goal(self) -> str:
        """
        Prompts the user to input their learning goal and returns it as a string.
        Returns:
            str: The user's learning goal.
        """
    ...
    def _save_report(self, topic_keywords: str, narrative_report: str):
        ...
        report_content = [
            f"# Knowledge Path: {report_title}",
            f"_Report generated by TALOS on {datetime.now().strftime('%d-%m-%Y %H:%M')}._",
            "\n---\n",
            narrative_report
        ]
        try:
