...
Description:
    ...
    constant step-size alpha). Each arm corresponds to a target Python module executed
    as a subprocess. If a target crashes (non-zero exit), the stderr is sent to the
    ...
    timestamped Markdown crash reports under reports/autonomous_tester/. A Synapse
...
# -- Target arms: (display_name, relative_path_from_project_root, python_args) --
TARGET_ARMS: List[Tuple[str, str, List[str]]] = [
    ("src/api/main_api.py (FastAPI Server)", "src/api/main_api.py", ["--help"]),
    ("src/mcp_server.py (MCP Server)", "src/mcp_server.py", ["--help"]),
    ("src/ingestion/daily_search.py (Daily Search)", "src/ingestion/daily_search.py", ["--help"]),
    ("src/analysis/citation_analyzer.py (Citation Analyzer)", "src/analysis/citation_analyzer.py", ["--help"]),
]
...
*Report generated by TALOS v5.9.0 Autonomous System Tester (RL-Driven Chaos Engineering)*
"""
...
def _run_test_cycle(
    ...
    if crashed:
        ...
        console.print(f"[dim green]Crash report saved: {report_path}[/dim green]")
...
def run_autonomous_tester(cycles: int = 10):
    ...
    console.print(
        Panel(
            Text(
                f"Test run complete. Q-table saved to {Q_TABLE_PATH}.\n"
                f"Components with positive fragility: {crashes_found}/{len(TARGET_ARMS)}.\n"
                f"Crash reports: {REPORTS_DIR}",
                style="white",
            ),
