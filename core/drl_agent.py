# -*- coding: utf-8 -*-
"""
Module: drl_agent.py (v1.1 — CuDNN fix: flatten_parameters)
...
Description:
    ...
    query next. Uses experience replay (replay memory) to break correlations
    between consecutive observations, ε-greedy exploration to discover new
    strategies, and a target network (DDQN) to stabilise learning.
    ...
    - LSTM layers (3 stacked) capture temporal patterns — the agent remembers
      ...
      exhaust its limit.
    - Soft target updates (τ = 1e-3) give smooth, stable training without
      the abrupt jumps of periodic hard updates.
    - Hidden states are reset at the start of each episode so the agent
      does not carry stale memory into a new "day".
    - flatten_parameters() is called before every LSTM forward pass to
      reset CuDNN memory pointers, preventing the "cudnn RNN backward can
      only be called in training mode" error.
"""
import torch as T
...
# ── Hyperparameters (tuned for the TALOS 3-source + sleep problem) ──────────
...
# ── TALOS-specific dimensions ────────────────────────────────────────────────
STATE_SPACE = 6        # The 6-element observation from TalosEnv
ACTION_SPACE = 4       # ArXiv, OpenAlex, Semantic Scholar, Sleep
...
class DuelingLSTM(nn.Module):
    ...
    def __init__(self, input_dim=STATE_SPACE, output_dim=ACTION_SPACE):
        ...
        Args:
            input_dim (int): Size of observation vector (6 for TalosEnv).
            output_dim (int): Number of possible actions (4).
        """
        ...
        # A(s,a): advantage — outputs 4 values, one per action
    ...
    def forward(self, state):
        ...
        For a single step, batch=1, seq_len=1, features=6.

        Args:
            state (torch.Tensor): Input tensor of shape (B, L, F).

        Returns:
            torch.Tensor: Q-values of shape (B, 1, 4).
        """
        # ── LSTM layer 1 ───────────────────────────────────────────────────
...
class TalosDRLAgent:
    ...
    that maximises the cumulative reward from the TalosEnv environment.
    ...
    def __init__(self):
        """
        Initialise the DDQN agent with online and target networks.
        """
        ...
        self.actor_online = DuelingLSTM(STATE_SPACE, ACTION_SPACE).to(DEVICE)
        self.actor_target = DuelingLSTM(STATE_SPACE, ACTION_SPACE).to(DEVICE)
    ...
    def reset_hidden_states(self):
        ...
        # sequence is passed. We pass state with shape (1, 1, 6) each
    ...
    def act(self, state, eps=0.0):
        ...
        Args:
            state (np.ndarray): Current observation (6-element vector).
        ...
        # Shape: (1, 1, 6) — batch=1, sequence_length=1, features=6
        ...
        if random.random() > eps:
            ...
            # torch.no_grad() is essential for performance (avoids building
            # the computation graph for inference). Works correctly with
            # CuDNN because flatten_parameters() is called in forward().
            with T.no_grad():
    ...
    def learn(self):
        ...
        # Each state was stored as a flat (6,) array. The LSTM expects
        # shape (batch, seq_len, features), so we unsqueeze to (B, 1, 6).
    ...
    def save(self, path):
        ...
        Args:
            path (str): File path to save to (e.g., 'models/talos_drl.pth').
        """
        T.save(self.actor_online.state_dict(), path)
    ...
    def load(self, path):
        ...
        """
        self.actor_online.load_state_dict(T.load(path, map_location=DEVICE))
        self.actor_target.load_state_dict(self.actor_online.state_dict())